...
- Cron-триггер (`scheduled`) вызывает `runReminders()`; расписание групп —
  таблица Supabase `bot_groups` (миграция `0022_bot_groups_table.sql`, зеркало
  `website/schedule.html`, включая гитару Вт/Чт/Сб 17:00 — Георгий Захаров).
  Правки расписания — SQL/Supabase Dashboard, без деплоя воркера.
