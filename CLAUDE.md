...
- Подключено на главной: заголовок и подзаголовок героя + 10 заголовков разделов (ключи `home.*`). **Расширение:** повесить `data-tx="page.slot"` в HTML нужной страницы + добавить строку в `TEXT_SLOTS` — новые тексты сразу редактируемы.
