...
## Phase 6 — Надёжность напоминаний бота (эскалация, тихие часы, «Моё расписание»)

Всё в воркере `workers/lead-forwarder.js`, дополнительных миграций не требует (использует существующие `bot_students`/`bot_attendance`/`bot_groups`).

- **Эскалация по группам:** в `runReminders`, если за `ESC_H` (=3) часа до занятия родитель не ответил на опрос «придёт/не придёт», а напоминание уже слали — владельцу уходит сводка «📞 Не подтвердили — стоит позвонить». Разовый флаг — строка `bot_attendance` с `kind='esc'` (уникальность (student, date, group, kind) не даёт дублей). Родитель может замьютить бота — но неявка не будет тихой: её ловит звонок.
- **Тихие часы:** `QUIET_START`=22, `QUIET_END`=8 (Almaty). В окне `runReminders` сразу `return 0` — ничего не шлёт и НЕ помечает отправленным, всё уходит первым прогоном после 08:00. Снижает раздражение и поводы замьютить.
- **Меню бота:** добавлена кнопка «📅 Моё расписание» (`my:sched` → `sendMySchedule`) — дни/время/преподаватель группы каждого ребёнка + ближайшее занятие (bot_students × bot_groups).
- **Нельзя технически:** запретить пользователю мьютить бота или форсировать звук — Telegram (как и любая платформа) отдаёт управление уведомлениями получателю. Единственная надёжная страховка — эскалация на звонок (см. выше).

### Быстрый вход в приложение — Main Mini App (настройка в @BotFather, не код)
Чтобы `@artshpacebot` открывался как приложение в один тап (и по ссылке `t.me/artshpacebot?startapp`): @BotFather → /mybots → выбрать бота → Bot Settings → Configure Mini App → включить и указать URL `https://artshpace.github.io/bot1/website/index.html`. Автозапуск приложения по нажатию «Запустить»/`/start` Telegram НЕ поддерживает — `/start` намеренно оставлен как приветствие с кнопками (в т.ч. «Открыть приложение» и подключение напоминаний).
